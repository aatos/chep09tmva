...
\begin{itemize}
\item Moderate customisation was required to map our problem into TMVA framework
\end{itemize}
\end{itemize}
\vspace{0.5cm}
...
\\($m_{H_{\pm}}=200 GeV/c^2$, $\tan\beta = 30$, max. $m_H$ SYSY scenario \footnote{\href{http://arxiv.org/pdf/hep-ph/9912223}{arXiv:hep-ph/9912223}}.)
...
\vspace{0.1cm}
Our analysis is based on bollowing variables:
...
At Monte Carlo level following preselection cuts were used:
\begin{itemize}
\item {\tt jetEt $>$ 100}
...
distribution of jet $E_T$ ({\tt jetEt}) and jet $\eta$ ({\tt jeteta})}.
%\end{minipage}
\label{fig:variables}
\end{figure}
}
...
[Material provided by Matti to be added]
}
...
\begin{itemize}
\item {\bf ANN:} Of three Multilayer Perceptrons (MLP) implementations supported by TMVA
...
\begin{itemize}
\item Systematic uncertainty was estimated by repeating the bull analysis with independent background data
\end{itemize}
\end{itemize}
...
{\color{dark-blue} Fig.~2: Convergence of ANN over 1000 training cycles.}
%\end{minipage}
\label{fig:variables}
\end{figure}
\begin{center}
...
\begin{figure}[h]
  \begin{center}
    \includegraphics[width=.55\textwidth]{images/mk_svm_gauss}
  \end{center}
%  \caption{}
{\color{dark-blue}The output of the SVM classifier with Gaussian kernel}.
...
\end{itemize}
\item Some customisation was needed to cast our problem into TMVA framework
...
and are planning to perform additional research in this area
